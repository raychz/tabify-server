...
export class UserSetting extends TabifyBaseEntity{
    ...
    @OneToOne(type => PaymentMethod, paymentMethod => paymentMethod.userSetting ,{ nullable: true }) // Check if the user changes a Payment Method (only removes it) but does not delete the payment method
    @JoinColumn()
